...
import { buildInquestQuery, getInquestKeywords } from '../../../common/services/inquestApi';
...
export default function SearchInquests(props) {
  ...
  useEffect(() => {
    const fetchKeywords = async () => {
      const response = await getInquestKeywords();
  ...
  const handleKeywordsChange = newSelectedKeywords => {
    ...
    onQueryChange(buildInquestQuery(newSelectedKeywords));
  };
